...
class MostEmailedTableViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        NetworkServiseManager().alamo(source: mostEmailedUrlString) { (model) in
...
extension MostEmailedTableViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return counter[mostEmailedUrlString]!
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        _ = NetworkServiseManager().alamo(source: mostEmailedUrlString, { (model) in
