...
class MostSharedTableViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        NetworkServiseManager().alamo(source: mostSharedUrlString) { (model) in
...
extension MostSharedTableViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return counter[mostSharedUrlString]!
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        _ = NetworkServiseManager().alamo(source: mostSharedUrlString, { (model) in
